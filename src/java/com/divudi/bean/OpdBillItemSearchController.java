...
public class OpdBillItemSearchController implements Serializable {
    ...
    public List<BillItem> getBillItemsAll() {
        if (billItemsOwn == null) {
            ...
            if (txtSearch == null || txtSearch.trim().equals("")) {
                sql = "select bi from BillItem bi join bi.bill b join b.patient.person p where b.createdAt between :fromDate and :toDate order by bi.id desc";
                billItemsOwn = getBillItemFacade().findBySQL(sql, m, TemporalType.TIMESTAMP, 100);
            } else {
                sql = "select bi from BillItem bi join bi.bill b join b.patient.person p where (upper(p.name) like '%" + txtSearch.toUpperCase() + "%' or upper(b.insId) like '%" + txtSearch.toUpperCase() + "%' or p.phone like '%" + txtSearch + "%' or upper(bi.item.name) like '%" + txtSearch.toUpperCase() + "%' ) and b.createdAt between :fromDate and :toDate order by bi.id desc";
    ...
    public List<BillItem> getBillItemsOwn() {
        if (billItemsAll == null) {
            ...
            m.put("fromDate", fromDate);
            if (txtSearch == null || txtSearch.trim().equals("")) {
                sql = "select bi from BillItem bi join bi.bill b join b.patient.person p where b.institution.id = " + getSessionController().getInstitution().getId() + " and b.createdAt between :fromDate and :toDate order by bi.id desc";
                billItemsAll = getBillItemFacade().findBySQL(sql, m, TemporalType.TIMESTAMP, 100);
            } else {
                sql = "select bi from BillItem bi join bi.bill b join b.patient.person p where  b.institution.id = " + getSessionController().getInstitution().getId() + " and  (upper(p.name) like '%" + txtSearch.toUpperCase() + "%' or upper(b.insId) like '%" + txtSearch.toUpperCase() + "%' or p.phone like '%" + txtSearch + "%' or upper(bi.item.name) like '%" + txtSearch.toUpperCase() + "%' ) and b.createdAt between :fromDate and :toDate order by bi.id desc";
