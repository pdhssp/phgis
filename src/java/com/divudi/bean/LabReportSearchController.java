...
@Named
@SessionScoped
public class LabReportSearchController implements Serializable {
...
//        profTot = 0;
    ...
    public void calTotalsWithout(Institution ins, Department dep) {
        clearTotals();
        String sql;
        Map tm;
        ...
        if (dep != null) {
            sql = "select sum(f.total - f.staffFee) from Bill f where f.institution.id=" + ins.getId()
                    + " and f.toDepartment.id=" + dep.getId() + " and f.retired=false and type(f) = :billClass and f.billType = :billType and "
            ...
            sql = "select sum(f.total - f.staffFee) from Bill f where f.institution.id=" + ins.getId()
                    + " and  f.retired=false and type(f) = :billClass and f.billType = :billType and "
                    + "(f.paymentScheme.paymentMethod = :pm1 or f.paymentScheme.paymentMethod = :pm2 or f.paymentScheme.paymentMethod = :pm3  or f.paymentScheme.paymentMethod = :pm4)"
                    + " and  f.createdAt between :fromDate and :toDate ";
        }
...
//        profTot = 0;
        ...
        System.out.println("Pro : " + profTot);
        System.out.println("net Tot : " + labHandover);
...
//        profTot = 0;
    ...
    public List<Bill> getLabBillsOwn() {
        if (labBills == null) {
            ...
            String sql = "select f from Bill f where f.retired=false and f.billType = :billType and f.institution.id=" + getSessionController().getLoggedUser().getInstitution().getId() + " and f.createdAt between :fromDate and :toDate and f.toDepartment.id=" + getDepartment().getId() + " order by type(f), f.insId";
            Map tm = new HashMap();
            tm.put("fromDate", fromDate);
            tm.put("toDate", toDate);
            tm.put("billType", BillType.OpdBill);
    ...
    public List<Bill> getLabBillsWithoutOwn() {
        if (labBills == null && getDepartment() != null) {
            ...
            sql = "select f from Bill f where f.retired=false and f.billType = :billType and (f.paymentScheme.paymentMethod = :pm1 or f.paymentScheme.paymentMethod = :pm2 or f.paymentScheme.paymentMethod = :pm3 or f.paymentScheme.paymentMethod = :pm4 ) and f.institution.id=" + getSessionController().getInstitution().getId() + " and f.toDepartment.id=" + getDepartment().getId() + " and f.createdAt between :fromDate and :toDate order by type(f), f.insId";
            ...
            labBills = getBillFacade().findBySQL(sql, tm, TemporalType.TIMESTAMP);
            if (labBills!=null) {
                calTotalsWithout(getSessionController().getInstitution(), getDepartment());
            }else{
                clearTotals();
            }
        }
...
//        profTot = 0;
     ...
     */
    public LabReportSearchController() {
    }
